...
class Provider(ABC):
    ...
    async def is_available(self) -> bool:
        """Check if this provider is available. Override for async checks."""
        return True
...
class OllamaProvider(Provider):
    ...
    @property
    def available(self) -> bool:
        return True  # Actual check done in is_available
...
''',
    ...
    async def generate(
        ...
        try:
            ...
            # Fallback to pragmatist template on format errors
            template = self._TEMPLATES["pragmatist"]
            defaults = self._DEMO_SOLUTIONS["default"].get("pragmatist", {})
            code = template.format(
                func_name=func_name,
                params=params,
                param_names=params.split(":")[0].strip() if ":" in params else params,
...
def _extract_params(desc: str) -> str:
    """Extract parameters from challenge description."""
    desc_lower = desc.lower()
...
def _extract_rtype(desc: str) -> str:
    """Extract return type from challenge description."""
    desc_lower = desc.lower()
    ...
    if "string" in desc_lower or "text" in desc_lower:
        return "str"
    if "number" in desc_lower or "count" in desc_lower or "integer" in desc_lower:
        return "int"
    return "object"
